...
import { hashCanonical } from './chunk-TN5K7UDO.js';
...
var NativeFableWorkflowAdapter = class {
  ...
  pm;
  consult(jobId, consultationId, query, options) {
    return this.call("Answer one bounded noncritical question. Return only strict JSON with schema_version:1, consultation_id, answer, alternatives, uncertainties. Do not return actions, verdicts, execution instructions, passport updates, or merge advice.", { job_id: jobId, consultation_id: consultationId, purpose: query.purpose, question: query.question, verification_method: query.verification_method }, options);
  }
  async available() {
    const result = await capability("claude", "fable");
    return { available: result.available && result.unsupported_options.length === 0, detail: result.detail };
  }
  async call(instruction, projection, options) {
    const prompt = bounded(`${instruction}
...
${JSON.stringify(projection)}`, options.max_input_bytes);
    ...
    return { value: parseJson(result.text), session_mode: "none", usage: result.usage };
  }
};
...
var NativeWorkflowRoleResolver = class {
  ...
  async availability(binding, role) {
    if (!supports(binding, role)) return { available: false, detail: `Unsupported ${role} binding: ${binding.adapter}` };
  ...
  decide(binding, passport, stage, evidence, threadId) {
    assertSupported(binding, stage === "post_opus" || stage === "after_fable_post" ? "reviewer" : "supervisor");
    return this.codex.decide(withProfile(passport, "codex", binding), stage, evidence, threadId);
  }
  execute(binding, passport, prompt, workspace, sessionId, mode) {
    assertSupported(binding, "implementer");
    return this.implementer.execute(withProfile(passport, "opus", binding), prompt, workspace, sessionId, mode);
  }
  consult(binding, jobId, consultationId, query, options) {
    assertSupported(binding, "adviser");
    return this.adviser.consult(jobId, consultationId, query, { ...options, model: binding.profile.model, timeout_ms: binding.profile.timeout_ms });
  }
};
var NativeWorkflowGitGateway = class {
  ...
  async runChecks(worktree, commit, commands) {
    ...
    for (const command of trusted) {
      ...
      try {
        const { stdout, stderr } = await execFileAsync(executable, args, { cwd: worktree, env: buildChildEnv(), maxBuffer: 4 * 1024 * 1024 });
  ...
  async merge(branch, expectedCommit, targetBranch, baseCommit) {
    try {
      ...
      if (branchCommit !== expectedCommit) return { success: false, detail: "Workflow branch changed after review" };
      ...
      await git(this.projectRoot, ["merge", "--no-ff", expectedCommit, "-m", `Merge reviewed ${branch}`]);
...
async function capability(command, role = "opus") {
  ...
  try {
    ...
    const [{ stdout: version }, { stdout: help }] = await Promise.all([
      ...
      execFileAsync(command, ["--help"], { env, timeout: 5e3, maxBuffer: 1024 * 1024 })
    ]);
...
function describeCapability(adapter, command, role, version, help) {
  const claudeBase = ["--print", "--output-format", "--max-turns", "--model", "--effort"];
  const required = command === "claude" ? role === "fable" ? [...claudeBase, "--bare", "--tools", "--disable-slash-commands", "--strict-mcp-config", "--mcp-config", "--no-session-persistence"] : claudeBase : command === "codex" ? ["exec", "--json", "--sandbox", "--model"] : [];
  ...
  const roleCompatibility = Object.fromEntries(["supervisor", "implementer", "adviser", "reviewer"].map((candidate) => {
    const compatible = compatibleRoles.includes(candidate);
    const reasons = compatible ? [optionReason].filter((value) => value !== null) : [transportReason ?? `${adapter} is not compatible with the ${candidate} workflow role`];
    return [candidate, { compatible: compatible && reasons.length === 0, reasons }];
  }));
  ...
  return {
    ...
    structured_output: command === "codex" ? { supported: help.includes("--json"), format: "jsonl" } : command === "claude" ? { supported: help.includes("--output-format"), format: "stream-json" } : { supported: false, format: null },
...
function unavailableCapability(adapter, command) {
  ...
  const role_compatibility = Object.fromEntries(["supervisor", "implementer", "adviser", "reviewer"].map((role) => [role, { compatible: false, reasons: [reason] }]));
  return { adapter, command, installed: false, version: null, transport: command === "codex" || command === "claude" ? "stdin" : "unsupported", structured_output: { supported: false, format: null }, sandbox: { supported: false, mode: null }, tools: { configurable: false, mode: "unknown" }, resume: { advertised: false, enabled: false }, role_compatibility, supported_options: [], unsupported_options: [], detail: reason, available: false, advertised_native_resume: false, native_resume: false };
}
...
function supports(binding, role) {
  if (role === "supervisor" || role === "reviewer") return binding.adapter === "codex";
...
function assertSupported(binding, role) {
  if (!supports(binding, role)) throw new Error(`Unsupported ${role} binding: ${binding.adapter}`);
}
function withProfile(passport, key, binding) {
  return { ...passport, config: { ...passport.config, profiles: { ...passport.config.profiles, [key]: { ...passport.config.profiles[key], model: binding.profile.model, effort: binding.profile.effort, max_turns: binding.profile.max_turns, timeout_ms: binding.profile.timeout_ms } } } };
}
...
//# sourceMappingURL=native-adapters-22NLSRIR.js.map
//# sourceMappingURL=native-adapters-22NLSRIR.js.map