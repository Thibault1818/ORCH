...
import { validateRosterSnapshot, createRosterSnapshot, hashRosterSnapshot, isTerminalWorkflowPhase, hashCanonical, artifactReference, ARTIFACT_FILES, validateRosterAgent, hashRosterAgent, hashPersisted } from './chunk-TN5K7UDO.js';
...
var LegacyWorkflowRoleResolver = class {
  ...
  async availability(binding, role) {
    ...
    if (!supported) return { available: false, detail: `Unsupported ${role} binding: ${binding.adapter}` };
  ...
  execute(_binding, passport, prompt, workspace, sessionId, mode) {
    return this.ports.opus.execute(passport, prompt, workspace, sessionId, mode);
  }
...
var WorkflowEngine = class {
  ...
  async start(input) {
    ...
    const requiredChecks = validateDeterministicCheckCommands(input.required_checks ?? []);
    if (requiredChecks.length === 0) throw new Error("Workflow requires at least one meaningful deterministic check");
    ...
    const baseProfiles = { fable: { ...DEFAULT_WORKFLOW_CONFIG.profiles.fable, ...input.config?.profiles?.fable }, opus: { ...DEFAULT_WORKFLOW_CONFIG.profiles.opus, ...input.config?.profiles?.opus }, codex: { ...DEFAULT_WORKFLOW_CONFIG.profiles.codex, ...input.config?.profiles?.codex } };
    const roster = input.roster ? validateRosterSnapshot(input.roster, mode) : createRosterSnapshot({ supervisor: rosterAgent("codex", "codex", baseProfiles.codex), implementer: rosterAgent("claude", "opus", baseProfiles.opus), adviser: requestedCap === 1 ? rosterAgent("fable", "fable", baseProfiles.fable) : null }, mode);
    this.assertRuntimeRoster(roster, mode);
    const config = { fable_total_cap: requestedCap, max_input_bytes: input.config?.max_input_bytes ?? DEFAULT_WORKFLOW_CONFIG.max_input_bytes, max_output_bytes: input.config?.max_output_bytes ?? DEFAULT_WORKFLOW_CONFIG.max_output_bytes, passport_max_bytes: input.config?.passport_max_bytes ?? DEFAULT_WORKFLOW_CONFIG.passport_max_bytes, profiles: { fable: profileFromRoster(roster.adviser, baseProfiles.fable), opus: profileFromRoster(roster.implementer, baseProfiles.opus), codex: profileFromRoster(roster.supervisor, baseProfiles.codex) } };
    if (Boolean(roster.adviser) !== (config.fable_total_cap === 1)) throw new Error("Adviser binding and adviser call cap must be configured together");
    ...
    const capabilities = await Promise.all([this.roles.availability(roster.supervisor, "supervisor"), this.roles.availability(roster.implementer, "implementer"), this.roles.availability(reviewer, "reviewer"), ...roster.adviser ? [this.roles.availability(roster.adviser, "adviser")] : []]);
    ...
    const passport = { schema_version: 2, passport_revision: 1, job_id: id2, mode, current_revision: 1, objective: input.objective, current_phase: "codex_pre_opus", accepted_brief_hash: null, latest_implementation_brief: null, hard_constraints: [], acceptance_criteria: [], decisions: [], allowed_file_scope: input.allowed_file_scope ?? [], required_checks: trustedChecks, current_blockers: [], next_action: job.next_action, artifacts: [], active_worktree: null, target_branch: null, base_commit: null, current_commit: null, session_references: { codex: null, opus: null }, session_modes: { codex: "none", opus: "none" }, rotation_history: [], config, roster, roster_hash: rosterHash, active_roster: roster, active_roster_hash: rosterHash, roster_revision: 1, binding_rotation_history: [] };
  ...
  async codexDecision(job, stage) {
    ...
    const result = await this.invoke(job, semanticRole, "codex", binding, { stage, evidence }, () => this.roles.decide(binding, passport, stage, evidence, sameAsSupervisor ? sessions.codex_thread_id : null));
    let decision;
    try {
  ...
  async fableConsultation(job) {
    ...
    try {
      ...
      const result = await this.fableCall(job, binding, options, { consultation_id: consultationId, query }, () => this.roles.consult(binding, job.job_id, consultationId, query, options));
  ...
  async dispatchOpus(job, instruction, correction = false) {
    ...
    if (!prepared.branch || !prepared.worktree || !prepared.target_branch || !prepared.base_commit) prepared = await this.git.prepare(job.job_id);
  ...
  async opusExecution(job) {
    ...
    const result = await this.invoke(job, "implementer", "opus", binding, { brief_hash: job.accepted_brief_hash }, () => this.roles.execute(binding, passport, prompt, job.worktree, mode === "native_resume" ? sessions.opus_session_id : null, mode));
  ...
  async reviewEvidence(job, stage) {
    ...
    return { evidence: await this.git.inspect(job.branch, job.worktree), checks: await this.payload(job, "test_results"), opus: await this.payload(job, "opus_report"), fable_advice: fableAdvice };
  }
  ...
  async rotateBinding(jobId, role, binding, reason) {
    const auditReason = reason.trim();
    if (!auditReason) throw new Error("Binding rotation requires a nonempty reason");
    const job = await this.requiredJob(jobId);
    if (job.phase !== "paused" && job.phase !== "blocked") throw new Error(`Cannot rotate bindings while workflow is ${job.phase}`);
    if (job.current_operation) throw new Error("Cannot rotate bindings while a workflow operation is reserved");
    if (job.blocker?.startsWith("LEGACY_SCHEMA:")) throw new Error("Legacy schema workflow bindings cannot be rotated");
    if (!job.resume_phase || job.resume_phase === "verification" || job.resume_phase === "merge_ready" || isTerminalWorkflowPhase(job.resume_phase)) throw new Error(`Cannot rotate bindings at ${job.resume_phase ?? job.phase}`);
    const passport = await this.requiredPassport(jobId);
    const sessions = await this.requiredSessions(jobId);
    const nextBinding = validateRosterAgent(binding);
    if (role === "adviser" && (nextBinding.profile.effort !== "low" || nextBinding.profile.max_turns !== 1)) throw new Error("Adviser binding must use low effort and one turn");
    ...
    if (!previous) throw new Error(`Cannot rotate an unauthorized ${role} binding`);
    if (sameBinding(previous, nextBinding)) throw new Error("Binding rotation must change the binding");
    const nextRoster = validateRosterSnapshot({ ...active, [role]: nextBinding }, passport.mode);
    const probes = [this.roles.availability(nextBinding, role)];
    if (role === "supervisor" && "same_as" in active.reviewer) probes.push(this.roles.availability(nextBinding, "reviewer"));
    ...
    const history = { role, previous_binding_hash: previous ? hashRosterAgent(previous) : null, new_binding_hash: hashRosterAgent(nextBinding), previous_binding: previous, new_binding: nextBinding, reason: auditReason, timestamp, revision };
    ...
    const sessionRotation = sessionRole ? { role: sessionRole, previous_id: previousId, next_id: null, reason: `binding rotation: ${auditReason}`, timestamp } : null;
    const updatedSessions = { ...sessions, sessions_revision: sessions.sessions_revision + 1, ...sessionRole ? { [sessionKey]: null } : {}, ...role === "implementer" ? { opus_brief_hash: null } : {}, modes: sessionRole ? { ...sessions.modes, [sessionRole]: "none" } : sessions.modes, rotation_history: sessionRotation ? [...sessions.rotation_history, sessionRotation] : sessions.rotation_history, updated_at: timestamp };
    const profileKey = role === "supervisor" ? "codex" : role === "implementer" ? "opus" : role === "adviser" ? "fable" : null;
    const config = profileKey ? { ...passport.config, fable_total_cap: role === "adviser" ? 1 : passport.config.fable_total_cap, profiles: { ...passport.config.profiles, [profileKey]: { ...passport.config.profiles[profileKey], model: nextBinding.profile.model, effort: nextBinding.profile.effort, max_turns: nextBinding.profile.max_turns, timeout_ms: nextBinding.profile.timeout_ms } } } : passport.config;
    const updatedPassport = { ...passport, passport_revision: passport.passport_revision + 1, active_roster: nextRoster, active_roster_hash: hashRosterSnapshot(nextRoster), roster_revision: revision, binding_rotation_history: [...passport.binding_rotation_history, history], config, session_references: { codex: updatedSessions.codex_thread_id, opus: updatedSessions.opus_session_id }, session_modes: updatedSessions.modes, rotation_history: updatedSessions.rotation_history };
  ...
  async fableCall(job, binding, options, request, call) {
    try {
      return await this.invoke(job, "adviser", "fable", binding, request, call);
    } finally {
      await fs.rm(options.workspace, { recursive: true, force: true });
    }
  }
  async invoke(job, semanticRole, usageRole, binding, request, call) {
    const invocationId = this.invocation(job);
    ...
    try {
      ...
      const receipt = { schema_version: 2, job_id: job.job_id, invocation_id: invocationId, phase: job.phase, role: usageRole, semantic_role: semanticRole, roster_hash: passport.active_roster_hash, roster_revision: passport.roster_revision, binding_hash: bindingHash, role_adapter: binding.adapter, request_hash: requestHash, request, result_hash: hashPersisted(result), workflow_revision: job.revision, timestamp: (/* @__PURE__ */ new Date()).toISOString(), result };
  ...
  async runChecksOnce(job, worktree, commit2, commands) {
    const trusted = await this.git.validateChecks(validateDeterministicCheckCommands(commands), worktree);
    return this.effect(job, "checks", { worktree, commit: commit2, commands: trusted }, validateCheckResults, () => this.git.runChecks(worktree, commit2, trusted));
  }
  async ensureTrustedChecks(jobId) {
    const passport = await this.requiredPassport(jobId);
    await this.git.validateChecks(validateDeterministicCheckCommands(passport.required_checks), passport.active_worktree ?? void 0);
  }
  async mergeOnce(job, branch, commit2, targetBranch, baseCommit) {
    return this.effect(job, "merge", { branch, commit: commit2, targetBranch, baseCommit }, validateMergeResult, () => this.git.merge(branch, commit2, targetBranch, baseCommit));
  }
  ...
  assertRuntimeRoster(roster, mode) {
    if (mode === "direct" && roster.adviser) throw new Error("Direct workflow roster cannot include an adviser");
  }
};
function usage() {
...
function rosterAgent(adapter, name, profile) {
  return { adapter, profile: { name, model: profile.model, effort: profile.effort, max_turns: profile.max_turns, timeout_ms: profile.timeout_ms } };
}
function profileFromRoster(binding, fallback) {
  return binding ? { model: binding.profile.model, effort: binding.profile.effort, max_turns: binding.profile.max_turns, timeout_ms: binding.profile.timeout_ms, permission_mode: fallback.permission_mode } : fallback;
}
...
function withoutSession(result) {
  return { ...result, session_id: void 0, session_mode: "none", resumed: false, resume_failed: false };
}
...
//# sourceMappingURL=chunk-HB4X2WT2.js.map
//# sourceMappingURL=chunk-HB4X2WT2.js.map