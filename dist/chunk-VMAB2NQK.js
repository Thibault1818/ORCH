...
import { validateRosterSnapshot, createRosterSnapshot, hashRosterSnapshot, isTerminalWorkflowPhase, hashCanonical, artifactReference, ARTIFACT_FILES, validateRosterAgent, hashRosterAgent, hashPersisted } from './chunk-77BIYQ4K.js';
...
var WorkflowEngine = class {
  ...
  async run(jobId) {
    ...
    while (true) {
      ...
      if (isTerminalWorkflowPhase(job.phase) || job.phase === "paused" || job.phase === "blocked" || job.phase === "awaiting_approval")
        return job;
    }
  }
  async advance(jobId) {
  ...
  async approve(jobId, reason) {
    ...
    const checkArtifact = await this.store.readArtifact(job.job_id, "test_results");
    ...
    const existing = await this.store.readArtifact(job.job_id, "human_approval");
    if (existing) {
      ...
      const passport2 = await this.requiredPassport(job.job_id);
      const registered = passport2.artifacts.some((item) => item.filename === existing.metadata.filename && item.hash === existing.metadata.artifact_hash);
  ...
  async verification(job) {
    ...
    if (!checks.passed || !sameCommands(checks.checks.map((check) => check.command), passport.required_checks) || checks.checks.length === 0 || !hasMeaningfulChecks(checks.checks.map((check) => check.command)) || evidence.commit !== job.current_commit || evidence.diff_hash !== job.reviewed_diff_hash)
      return this.block(
        job,
  ...
  async merge(job) {
    ...
    const approval = validateHumanApproval(await this.payload(job, "human_approval"));
    const approvedChecks = await this.store.readArtifact(job.job_id, "test_results");
...
export { DEFAULT_WORKFLOW_CONFIG, LegacyWorkflowRoleResolver, WORKFLOW_SCHEMA_VERSION, WorkflowEngine, hasMeaningfulChecks, validateCheckResults, validateCodexDecision, validateFableAdvice, validateFableFallbackRecord, validateFableQuery, validateHumanApproval, validateOpusResult };
//# sourceMappingURL=chunk-VMAB2NQK.js.map
//# sourceMappingURL=chunk-VMAB2NQK.js.map