...
function artifact(value, label) {
  ...
  return { filename, hash: hash(o.hash, `${label}.hash`), phase: phase(o.phase), revision: integer(o.revision, `${label}.revision`, 1), iteration: integer(o.iteration, `${label}.iteration`, 1), role: enumeration(o.role, ["codex", "fable", "opus", "orchestrator", "human"], `${label}.role`) };
}
...
// src/infrastructure/workflow/state-migrations.ts
var WORKFLOW_SCHEMA_VERSION = 2;
function workflowStateVersion(value, label) {
  const raw = object2(value, label);
  if (raw.schema_version === 1 || raw.schema_version === 2) return raw.schema_version;
  if (Number.isSafeInteger(raw.schema_version) && raw.schema_version > WORKFLOW_SCHEMA_VERSION)
...
function migrateWorkflowState(jobValue, passportValue, sessionsValue) {
  const versions = [
  ...
  const legacyPassport2 = validateWorkflowPassport(passportValue);
...
function validateLegacyNestedState(passportValue, sessionsValue) {
  const passport = object2(passportValue, "legacy workflow passport");
  for (const field of [
    "hard_constraints",
    "acceptance_criteria",
    "allowed_file_scope",
  ...
  const sessions = object2(sessionsValue, "legacy workflow sessions");
  if (sessions.recorded_invocations !== void 0 && (!Array.isArray(sessions.recorded_invocations) || sessions.recorded_invocations.some((item) => typeof item !== "string")))
...
function validateWorkflowMigrationJournal(value) {
  const raw = object2(value, "workflow migration journal");
...
var WorkflowArtifactStore = class {
  ...
  constructor(projectRoot, options = {}) {
    this.root = options.rootIsStateRoot ? path.join(projectRoot, "workflows") : path.join(projectRoot, ".orchestry", "workflows");
  }
  get rootPath() {
    return this.root;
  }
  ...
  async readJob(jobId) {
    const id2 = safeId(jobId);
    await this.ensureMigration(id2);
    await this.recoverSessions(id2);
    await this.recoverTransition(id2);
  ...
  async ensureMigration(id2) {
    const active = this.migrations.get(id2);
    if (active) return active;
    const migration = this.migrateOrRecover(id2).finally(() => {
      this.migrations.delete(id2);
    });
    this.migrations.set(id2, migration);
    return migration;
  }
  async migrateOrRecover(id2) {
    const pending = this.file(id2, "migration.pending.json");
    const rawJournal = await readJson(pending);
    if (rawJournal) {
      await this.applyMigration(id2, validateWorkflowMigrationJournal(rawJournal));
      return;
    }
    const [job, passport, sessions] = await Promise.all([
      readJson(this.file(id2, "job.json")),
      readJson(this.file(id2, "passport.json")),
      readJson(this.file(id2, "sessions.json"))
    ]);
    if (job === null && passport === null && sessions === null) return;
    if (job === null || passport === null || sessions === null)
      throw new Error(`Workflow state is incomplete: ${id2}`);
    const versions = [
    ...
    if (!versions.every((version) => version === 1))
      throw new Error(`Workflow state has mixed schema versions without a migration journal: ${id2}`);
    const journal = migrateWorkflowState(job, passport, sessions);
    await this.write(pending, journal);
    await this.applyMigration(id2, journal);
  }
  async applyMigration(id2, journal) {
    const pending = this.file(id2, "migration.pending.json");
    const targets = [
    ...
    for (const [name, target, label] of targets) {
      const file = this.file(id2, name);
      const current = await readJson(file);
      if (current !== null && workflowStateVersion(current, label) === 2) {
        const validated = name === "job.json" ? validateWorkflowJob(current) : name === "passport.json" ? validateWorkflowPassport(current) : validateWorkflowSessions(current);
        if (canonicalJson2(validated) !== canonicalJson2(target))
...
//# sourceMappingURL=chunk-77BIYQ4K.js.map
//# sourceMappingURL=chunk-77BIYQ4K.js.map