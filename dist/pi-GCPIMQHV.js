...
// src/infrastructure/adapters/pi.ts
var PiAdapter = class {
...
function createPiRpcEvents(command, processManager, stderrTail, signal) {
  async function* generate() {
    ...
    if (completion.spawnError && !signal?.aborted && !gotDoneEvent) {
      ...
      const classified = classifyAdapterError(message, completion.exitCode ?? void 0);
      throw Object.assign(new Error(message), { errorKind: classified });
    }
    if (!completion.ok && !signal?.aborted && !gotDoneEvent) {
      const baseMsg = completion.integrityError ?? (completion.termination === "timed_out" ? "Pi process timed out" : `Pi process exited with code ${completion.exitCode}`);
...
export { PiAdapter };
//# sourceMappingURL=pi-GCPIMQHV.js.map
//# sourceMappingURL=pi-GCPIMQHV.js.map