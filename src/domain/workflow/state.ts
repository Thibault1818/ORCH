...
export interface WorkflowDecision { invocation_id: string; action: string; summary: string; provenance: 'codex'; timestamp: string; }
...
export interface WorkflowSessionsV2 { schema_version: 2; job_id: string; codex_thread_id: string | null; opus_session_id: string | null; opus_brief_hash: string | null; modes: Record<'codex' | 'opus', SessionMode>; rotation_history: SessionRotation[]; recorded_invocations: string[]; usage: Record<'codex' | 'fable' | 'opus', AgentUsage>; updated_at: string; }
...
export interface WorkflowInvocationReceiptV2 { schema_version: 2; job_id: string; invocation_id: string; phase: WorkflowPhase; role: 'codex' | 'fable' | 'opus'; request_hash: string; workflow_revision: number; timestamp: string; result: unknown; }
