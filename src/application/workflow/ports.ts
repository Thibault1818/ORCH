...
export interface WorkflowRoleResolver {
  availability(binding: RosterAgent, role: SemanticRole): Promise<{ available: boolean; detail: string }>;
...
export interface WorkflowRuntimePorts { roles: WorkflowRoleResolver; git: WorkflowGitPort; }

export class LegacyWorkflowRoleResolver implements WorkflowRoleResolver {
  constructor(private readonly ports: Pick<WorkflowRolePorts, 'codex' | 'fable' | 'opus'>) {}

  async availability(binding: RosterAgent, role: SemanticRole) {
    const supported = role === 'supervisor' || role === 'reviewer'
  ...
  decide(_binding: RosterAgent, passport: WorkflowPassportV2, stage: CodexDecisionStage, evidence: CodexDecisionEvidence, threadId: string | null) { return this.ports.codex.decide(passport, stage, evidence, threadId); }
  execute(_binding: RosterAgent, passport: WorkflowPassportV2, prompt: string, workspace: string, sessionId: string | null, mode: 'new' | 'native_resume' | 'passport_handoff') { return this.ports.opus.execute(passport, prompt, workspace, sessionId, mode); }
  consult(_binding: RosterAgent, jobId: string, consultationId: string, query: FableQueryV1, options: FableCallOptions) { return this.ports.fable.consult(jobId, consultationId, query, options); }
}
