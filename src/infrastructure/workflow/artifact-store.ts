...
export const ARTIFACT_FILES = {
  codex_brief: 'codex-brief-r%REV%-i%ITER%-a%SEQ%.json', fable_plan: 'fable-plan-r%REV%-i%ITER%-a%SEQ%.json', codex_plan_review: 'codex-plan-review-r%REV%-i%ITER%-a%SEQ%.json',
  fable_final_prompt: 'fable-final-prompt-r%REV%-i%ITER%-a%SEQ%.md', opus_report: 'opus-report-r%REV%-i%ITER%-a%SEQ%.json', opus_diff: 'opus-r%REV%-i%ITER%-a%SEQ%.diff',
  test_results: 'test-results-r%REV%-i%ITER%-a%SEQ%.json', codex_technical_review: 'codex-technical-review-r%REV%-i%ITER%-a%SEQ%.json',
  fable_compliance_review: 'fable-compliance-review-r%REV%-i%ITER%-a%SEQ%.json', codex_synthesis: 'codex-synthesis-r%REV%-i%ITER%-a%SEQ%.json',
} as const;
...
export class WorkflowArtifactStore {
  ...
  async writeInvocationReceipt(value: WorkflowInvocationReceiptV1): Promise<void> { const id = safeId(value.job_id); const file = this.file(id, `invocations/${safeId(value.invocation_id)}.json`); try { await fs.access(file); return; } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; } await this.write(file, value); }
  async readInvocationReceipt(jobId: string, invocationId: string): Promise<WorkflowInvocationReceiptV1 | null> { return readJson<WorkflowInvocationReceiptV1>(this.file(safeId(jobId), `invocations/${safeId(invocationId)}.json`)); }
